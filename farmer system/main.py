...
class Register(db.Model):
    ...
    farming=db.Column(db.String(50),unique=True)
...
@login_required
def farmerdetails():
    query = Register.query.filter_by(farmername=current_user.username).all()
...
def addagroproduct():
    if request.method=="POST":
        ...
        products=Addagroproducts(username=username,email=email,phonenumber=phonenumber,productname=productname,productdesc=productdesc,price=price)
...
from flask_login import current_user  # Assuming you are using Flask-Login for user management

# Assuming signup_name is defined somewhere in your application configuration
signup_name = "admin"  # Replace with your actual signup name or identifier

@app.route('/register', methods=['POST', 'GET'])
@login_required
def register():
    farming = Farming.query.all()
    regi = Register.query.all()
    ...
    if request.method == "POST":
        ...
        if any(r.farming == farmingtype for r in regi):
            flash("Farming Type Already Exists")
        else:
            query = Register(
